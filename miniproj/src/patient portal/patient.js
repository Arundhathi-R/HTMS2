...
const PatientPortal = () => {
  ...
  const handleApptHistoryClick = () => {
    navigate('/apptHistory');
  };

  const handleScroll = (direction) => {
    if (direction === 'left') {
      setVisibleDoctors(prev => Math.max(prev - 1, 1));
    } else {
      setVisibleDoctors(prev => Math.min(prev + 1, doctors.length));
    }
  };
  ...
  return (
    <div className="patient-portal-container">
      <button className="logout-p" onClick={handleLogout}>Logout</button>
      <h1 className="patient-title">PATIENT</h1>
      <div className="image-upload-container">
        <label htmlFor="image-upload" className="image-upload-label">
          {selectedImage ? (
      ...
      <br />
      <button className="appointment-history-button" onClick={handleApptHistoryClick}>Appointment History</button>
      <h2 className="choose-doctor-title">Choose your doctor:</h2>
      <div className="doctor-carousel">
        {doctors.slice(0, visibleDoctors).map((doctor) => (
